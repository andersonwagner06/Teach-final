            ...
            <div class="card-body px-lg-5 py-lg-5">
              <form
                ...
                <div
                  ...
                  <p class="m-0 nav-link font-weight-light"
                     @click="resendEmail()">
                    Você não recebeu o email?<br>
                    Clique aqui para reenviá-lo.
                  </p>
                </div>

                <div class="text-center">
                  <base-button :disabled="loggingIn"
                               type="primary"
                               native-type="submit"
                               size="lg"
  ...
  methods: {
    ...
    resendEmail () {
      ...
      ApiService.post(`/auth/verify/email/resend/${this.userId}`)
        .then(res => {
          if (res.status === 200) {
            this.$notify({ type: 'success', message: `Um email de confirmação do registro foi reenviado!` })
          }
        })
        .catch(err => {
          this.$notify({ type: 'danger', message: `<b>Erro !</b> ${err.data.message || err} !` })
        })
    },
    getError (name) {
